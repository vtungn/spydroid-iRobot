...
public class RequestHandler {
	...
	static private void exec(JSONObject object, StringBuilder response) throws JSONException, IllegalArgumentException, IllegalAccessException {
		...
		else if (action.equals("iRoConn")) {
			if(iRC==null)
				iRC = new iRobotController();
			iRC.setupbluetoothConnect();
			iRC.getControlDev();
			
			response.append("[]");
		}
		else if (action.equals("iRoUp")) {
			iRC.drive("Forward",(char) 100,(char) 32768);
			response.append("[]");
		}
		else if (action.equals("iRoDown")) {
			iRC.drive("Forward",(char)-100,(char) 32768);
			response.append("[]");
		}
		else if (action.equals("iRoLeft")) {
			iRC.drive("Rotate",(char) 100,(char) 1);
			response.append("[]");
		}
		else if (action.equals("iRoRight")) {
			iRC.drive("Rotate",(char)-100,(char) 1);
			response.append("[]");
		}
		else if (action.equals("iRoStop")) {
			iRC.drive("Stop",(char) 0,(char) 0);
			response.append("[]");
		}
		else if (action.equals("Light1")){
...
//			     
	...
	public static boolean isON1=true;
	public static boolean isON2=true;
	public static boolean isON3=true;
}
class ClientThread implements Runnable {
    ...
    public void run() {
        ...
        try {
            ...
            RequestHandler.client = new Socket(serverAddr,SERVERPORT);
